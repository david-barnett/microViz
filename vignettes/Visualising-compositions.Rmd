...
```{r diet only}
ps <- dietswap %>% subset_samples(group == 'DI')

# subset to participants/"subjects" with samples for both timepoints
timepoint_groups <- sample_data(ps) %>% data.frame() %>% split.data.frame(f = .$timepoint.within.group)
...
# sample
sample_data(ps)$plot_groups <- interaction(sample_data(ps)$nationality, sample_data(ps)$timepoint.within.group)

sample_order <- sample_data(ps) %>% 
  data.frame() %>% 
  dplyr::arrange(timepoint.within.group, nationality, desc(subject)) %>% 
  rownames()
```
...
times_list <- ps %>% 
  plot_comp_bar(
    tax_level = 'Genus', n_taxa = 10, sample_order = sample_order,
    groups = 'plot_groups', bar_width = 0.7, label = "subject"
  )
