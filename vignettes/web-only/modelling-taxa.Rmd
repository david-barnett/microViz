...
*Parabacteroides*. We will transform the count data by first making it
...
directly uses un-transformed counts. By default, microViz's `tax_model()`
...
rate. This function simply applies the `p.adjust` function to (groups of) 
p values from the taxatree_stats dataframe.
...
lm_stats <- taxatree_stats_p_adjust(
  data = lm_stats, method = "BH", 
  grouping = "rank", # default setting
  # grouping = "taxon" # (adjust per taxon, a more relaxed setting)
  # grouping = NULL # (for no grouping, strictest setting)
)
...
## Multiple simple (regression) models

In the examples above we fit regression models with several predictor variables, 
e.g. in the form `taxon ~ disease + sex + antibiotics + steroids + age`.

But often we will want to see the results from multiple simple regression models, e.g. 
`taxon ~ disease`, `taxon ~ sex`, `taxon ~ antibiotics`, `taxon ~ steroids` and `taxon ~ age`

Since microViz version 0.9.4 it is now simple to do this. 
You need to pass a **list** of variables to the `variables` argument, or a vector of formulae to the `formula` argument.
...
simple_models <- phylo %>% 
  ...
  taxatree_models(
    type = lm, ranks = NULL, 
    variables = list("UC", "female", "antibiotics", "steroids", "age_scaled")
    # or equivalently
    # formula = paste("~", c("UC", "female", "antibiotics", "steroids", "age_scaled"))
  )
```

The structure of the taxatree_models list returned has one extra level of nesting, but you do not need to worry about this. 

```{r}
simple_models$taxatree_models$Phylum %>% str(max.level = 1)
simple_models$taxatree_models$Phylum$UC %>% str(max.level = 1)
```

You can extract a dataframe of statistics from the models in this nested list using `taxatree_models2stats`, in just the same way as before.

```{r}
simple_models_stats <- simple_models %>% 
...
simple_models_stats %>% 
  taxatree_plots(
    ...
    sig_stat = c("p.adj.BH.rank", "p.value"), sig_shape = c("cross", "circle"), 
    sig_size = c(1, 0.75), sig_stroke = c(0.5), sig_colour = "white"
  ) %>% 
  .[1:4] %>% 
