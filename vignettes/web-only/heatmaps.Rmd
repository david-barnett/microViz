...
The samples and taxa are sorted by similarity.
(*By default this uses hierarchical clustering with optimal leaf ordering, using
euclidean distances on the transformed data*).
...
You can easily swap to a symmetrical colour palette for transformations like
"clr" or "standardize". This is the default symmetrical palette but you can pick from many.
...
Positioning the heatmap legend at the bottom is possible. 
You can assign the heatmap to a name and then call `ComplexHeatmap`'s `draw` function.
...
metadata are possible. 
 ...
 - `anno_sample()` cannot have borders around each cell, but automatically adds a legend.

 - `anno_sample_cat()` can have cell borders, but can't draw a legend **YET**
...
Equivalent categorical annotations by two methods.
...
psqC %>% 
  comp_heatmap(
    grid_col = NA, cluster_rows = FALSE, row_title = NULL,
    ...
    sample_anno = sampleAnnotation(
      ...
      State2 = anno_sample_cat(
        var = "DiseaseState", border = T, col = cols, gp = grid::gpar(col = NA)
      )
    )
  ) %>% 
  ComplexHeatmap::draw(merge_legends = TRUE)
```


### Numbering cells

If you have fewer samples (and taxa) you might like to label the cells with
their values. By default, the raw counts are shown.
...
You can easily change to showing the same values as the colours by setting
`numbers_use_counts = FALSE`, and you can/should change the number of decimals shown too.
...
To demonstrate that coloration, numbering and seriation can all use different 
transformations of the original count data, the example below we specifies
seriating the taxa and samples using the same numerical values used for the numbers transformation, not the colours, which are just presence/absence!
