...
If we would try to aggregate at Genus or Family rank level, we discover that blank values at these ranks prevent taxonomic aggregation.

```{r tax_agg fail}
# tax_agg(pseq, rank = "Family") # this fails, and sends (helpful) messages about taxa problems
```

So we should run `tax_fix` first, which will fix most problems (with default settings), allowing taxa to be aggregated successfully (at any rank).
...
`tax_fix` finds searches all the ranks of the phyloseq object `tax_table` for:
