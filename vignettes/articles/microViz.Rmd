...
clr_pca <- ps %>%
  ...
  ord_calc("PCA") # Note: PCA is RDA without constraints (& ord_calc uses an RDA method to perform PCA)
```
...
tt_models <- ps %>%
  tax_filter(min_prevalence = 5 / 100, tax_level = "Genus") %>%
  taxatree_models(
    tax_levels = c("Phylum", "Family"), # skipping genera for speed of generating this example
