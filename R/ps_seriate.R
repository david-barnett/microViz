...
#'
#' Uses seriation methods from seriation::seriate and often calc_dist (depending on if seriation method requires a distance matrix)
#'
#' @param ps phyloseq object
#' @param method seriation method for ordering samples, from seriation::seriate
#' @param dist distance method for calc_dist (only used if required for particular seriation method!)
...
ps_seriate <- function(ps, method = "OLO_ward", dist = "bray", tax_transform = "identity", add_variable = FALSE) {
  if (phyloseq::nsamples(ps) > 2) {
    ...
    if (method %in% seriation::list_seriation_methods(kind = "matrix")) {
      ...
      distMat <- calc_dist(data = ps_transformed, dist = dist)[["distMat"]]
